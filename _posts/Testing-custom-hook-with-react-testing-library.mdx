...
To be able to progress in this tutorial , make make sure to install some packages
...
describe('Testing useNotes hook', () => {
  ...
  test('Update specific note by title', async () => {
    const { result } = renderHook(useNotes)
    expect(result.current?.searchResults).toStrictEqual(undefined)
    // update function from hook recive note object and extract id from it
