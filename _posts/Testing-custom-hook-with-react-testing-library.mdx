...
In this article, we will write a test for our useNotes hook, a simple hook where you can perform some actions like add, remove, and search for a note by title. We will use <Tag>react-testing/library</Tag> to test the app since the author of Enzyme said that Enzyme will not support React 18 (**[https://dev.to/wojtekmaj/enzyme-is-dead-now-what-ekl](https://dev.to/wojtekmaj/enzyme-is-dead-now-what-ekl)**).
...
The other approach is to test the hook in isolation, if our hook is more general and we want to write a unit test on it. For this approach, we will use the renderHook function so we can test our custom hook. If we call our hook directly inside the test, it will break the rules of React hooks, "you can't call a hook outside of a React component."
...
export const Notes = () => {
  ...
  const { notes, searchResults, AddNote, searchForNote } = useNotes()

  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const elements = e.currentTarget.elements
    if ('note' in elements && 'title' in elements) {
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        ...
        searchResults?.length === 0 ? (
          ...
          searchResults?.map((note) => (
            <Note note={note} key={note?.title + 'note'} />
          ))
        ) : (
          notes?.length >= 1 &&
          notes.map((note) => <Note note={note} key={note?.title + 'note'} />)
        )}
      </div>
    </div>
  )
...
const Note = ({ note }: { note: NoteShape }) => {
...
To be able to progress in this tutorial , make make sure to install some packages <Tag>react-tesing/libary</Tag> <Tag>react-dom/test-utils</Tag>

```tsx
yarn add react-tesing/libary react-dom/test-utils
```

so we have hook hold notes and functions for updating the notes ,so we have some use cases to test
...
describe('Testing useNotes hook', () => {
  ...
  // 4) update note using id
  test('search  for specific note by title', async () => {
    const { result } = renderHook(useNotes)
    expect(result.current?.searchResults).toStrictEqual(undefined)
    act(() => result.current.updateNote(noteForTest))
    act(() => result.current.searchForNote(noteForTest.title))
...
test('useNotes inside a component', async () => {
  ...
  userEvent.type(await screen.findByLabelText(/title/i), noteForTest.title)
  userEvent.type(await screen.findByLabelText(/note/i), noteForTest.note)
