...
Hey Folks, I think every app has a form that is public and anyone can fill the form without any token, so some guys try to do bad things with those forms, they create bots that fill these forms for purposes like SEO purposes to provide backlinks or add malware as well as check for the site are vulnerabilities upload, there are some solutions like recaptcha, but guys who have time to do the recaptcha ……
...
There are alternatives. Honeypots are additional inputs you put on a form to make bots think they are submitting the correct info. The thing is that we create inputs that look legit and hide them with CSS, and we check if these inputs are filled or not. If they're filled, it means there's a bot here. It's not a 100% solution, but with rate limiting on the server and some other methods, I think it's serving the purpose.
...
here in the handleSubmit function, we checked if the input we hide it is filled with text, and if it’s filled know it’s bot, so we can display the error or wherever you want
