...
Here, <Tag>P</Tag> is a variable that represents a property key of the original type <Tag>K</Tag>. We can customize <Tag>P</Tag> as we want; we can use it as is or create literal strings. <Tag>T</Tag> is the type that the property will be transformed into. Here, we can use it to create a new type.
...
P in K means, that the key property in an object will match one of the K arguments and T is the type of value can object has T can be anything in this example but we can put some constraints
